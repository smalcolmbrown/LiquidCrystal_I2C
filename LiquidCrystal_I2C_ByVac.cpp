...
//  Wire.write(0x03); 					//  ByVac command code 0x03 for backlight
  I2CWRITE(0x03); 					//  ByVac command code 0x03 for backlight
  if (value==0) {
//	Wire.write(1);
	I2CWRITE(1);	
  } else {
//	Wire.write((byte)0); 	// 1 for off since polarity is NEGATIVE
	I2CWRITE((byte)0);
  }
...
//  Wire.write(0x05); 					//  ByVac command code 0x05 for contrast
  I2CWRITE(0x05); 					//  ByVac command code 0x05 for contrast
  if (value==0) {
//	Wire.write((byte)0); 
	I2CWRITE((byte)0);
  }else {
...
//  Wire.write(mode+1); // map COMMAND (0) -> ByVac command code 0x01/ DATA  (1) ->  ByVac command code 0x02
  I2CWRITE(mode+1); // map COMMAND (0) -> ByVac command code 0x01/ DATA  (1) ->  ByVac command code 0x02
//  Wire.write(value);
  I2CWRITE(value);
  Wire.endTransmission();
}
